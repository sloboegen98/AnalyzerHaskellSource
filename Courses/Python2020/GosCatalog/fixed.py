    ...
    with ZipFile('data-4-structure-3.json.zip') as myzip:
        for name in myzip.namelist():
            with myzip.open(name) as myfile:
                ...
                # 2020-04-03 Ruslan Nasonov: это не 1%, а 10 элементов
                count_extracted = len(data) / 100
                for_insert.extend(sample(data, count_extracted))
...
class MuseumReport:
    ...
    def group_by_city(self):
        result_table = []
        # 2020-04-03 Ruslan Nasonov: все группировки лучше делать на стороне базы данных
    ...
    def get_route(self):
        # 2020-04-03 Ruslan Nasonov: и маршруты тоже можно посчитать в бд, там есть геозапросы
        route = ""
    ...
    def musfilter(self):
        # 2020-04-03 Ruslan Nasonov: сгруппировать по городам в базе тоже можно
...
class MuseumsSearcher:
    exhibits = dict()
    collection = pymongo.collection.Collection # 2020-04-03 Ruslan Nasonov: странная инициализация, Collection это класс
    person = ""
    ...
    def process_row(self, row):
            ...
            # 2020-04-03 Ruslan Nasonov: а еще лучше искать полнотекстовым поиском
    ...
    def create_museums_list(self):
        ...
        # 2020-04-03 Ruslan Nasonov: Тут итерация по всей коллекции экспонатов? Их же очень много.
